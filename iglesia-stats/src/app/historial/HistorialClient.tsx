...
export default function HistorialClient({ encuentros, campuses, profile }: {
    ...
    () =>
      encuentros.filter((e) => {
        ...
        if (search) {
          ...
          return (
            e.nombre_encuentro?.toLowerCase().includes(q) ||
            e.predicador?.toLowerCase().includes(q) ||
            e.nombre_mensaje?.toLowerCase().includes(q)
          )
        }
        return true
      }),
  ...
  function exportCSV() {
    const headers = [
      ...
      'Predicador',
      'Mensaje',
      'Asistencia Auditorio',
      'Kids',
      'Tweens',
      'Sala bebé',
      'Sensorial',
      'Aceptaron Jesús Presencial',
      'Total General',
      'Vol. Servicio',
      'Vol. Técnica',
      'Vol. Worship',
    ...
    const rows = filtered.map((e) => {
      ...
      return [
        ...
        e.predicador,
        e.nombre_mensaje || '',
        e.asistencia_auditorio,
        e.asistencia_kids,
        e.asistencia_tweens,
        e.asistencia_sala_bebe,
        e.asistencia_sala_sensorial,
        e.acepto_jesus_presencial,
        e.total_general,
        e.vol_servicio,
        e.vol_tecnica,
        e.vol_worship,
        ...
        e.vol_merch,
        e.vol_amor_casa,
        e.vol_sala_sensorial,
        e.vol_punto_siembra,
        e.acepto_jesus_online,
        e.espectadores,
        e.lideres_voluntarios || '',
        e.adm_campus || '',
      ].join(',')
    })
              ...
              {filtered.map((e) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isSuperAdmin && (
                      <td className="py-3 px-4 text-gray-500">{campus?.name || '—'}</td>
                    )}
                    ...
                    <td className="py-3 px-4 text-right font-semibold text-gray-900">
                      {(e.asistencia_auditorio || 0).toLocaleString('es-CL')}
                    </td>
                    <td className="py-3 px-4 text-right font-semibold text-emerald-600">
                      {jesus.toLocaleString('es-CL')}
                    </td>
                    <td className="py-3 px-4">
                      <span className="px-2 py-0.5 rounded-full text-xs bg-brand-50 text-brand-700 font-medium">
                        {e.modalidad}
                      </span>
                    </td>
          ...
          >
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {selected.fecha
                    ? format(parseISO(selected.fecha), "EEEE dd 'de' MMMM yyyy", { locale: es })
                    : ''}
                </p>
              </div>
              <button
            ...
            <div className="p-5 space-y-5">
              ...
              {(selected.modalidad === 'Presencial' ||
                selected.modalidad === 'Presencial+Online') && (
                <div>
                    ...
                    {[
                      ...
                      <div key={l as string} className="bg-gray-50 rounded-xl p-3">
                        ...
                        <p className="text-lg font-bold text-gray-900">
                          {((v as number) || 0).toLocaleString('es-CL')}
                        </p>
                      </div>
                    ))}
                  </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ['Servicio', selected.vol_servicio],
                    ['Técnica', selected.vol_tecnica],
                    ['Worship', selected.vol_worship],
                    ['Kids', selected.vol_kids],
                    ['Tweens', selected.vol_tweens],
                    ['RRSS', selected.vol_rrss],
                    ['Seguridad', selected.vol_seguridad],
                    ['Cocina', selected.vol_cocina],
                    ['Oración', selected.vol_oracion],
                    ['Sala bebés', selected.vol_sala_bebes],
                    ['Conexión', selected.vol_conexion],
                    ['Merch', selected.vol_merch],
                    ['Amor casa', selected.vol_amor_casa],
                    ['Sensorial', selected.vol_sala_sensorial],
                  ...
                  <div className="space-y-2 text-sm">
                    {selected.lideres_voluntarios && (
                      <p>
                        ...
                        <span className="font-medium">{selected.lideres_voluntarios}</span>
                      </p>
                    )}
                    {selected.adm_campus && (
                      <p>
                        <span className="text-gray-500">Admins:</span>{' '}
                        <span className="font-medium">{selected.adm_campus}</span>
                      </p>
                    )}
                    {selected.observaciones && (
                      <p>
                        <span className="text-gray-500">Obs:</span> {selected.observaciones}
                      </p>
                    )}
                  </div>
                </div>
